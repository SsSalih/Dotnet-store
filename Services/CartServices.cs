...
public class CartService : ICartService
{
    ...
    public async Task TransferCartToUser(string username)
    {
        ...
        var CoockieCart = await GetCart(_httpContextAccessor.HttpContext?.Request.Cookies["cutomerId"]!);

        foreach (var item in CoockieCart?.CartItems!)
        {
        ...
        _context.Carts.Remove(CoockieCart);
        await _context.SaveChangesAsync();
    }
}
